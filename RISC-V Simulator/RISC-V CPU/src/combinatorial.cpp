...
AND::AND(std::string and_name = "and") {
    ...
    this->in1      = new port(this, RECEIVE_PORT, 1, and_name + "_in1");
    this->in2      = new port(this, RECEIVE_PORT, 1, and_name + "_in2");
    this->out      = new port(this, SEND_PORT,    1, and_name + "_out");
}
...
OR::OR(std::string or_name = "or") {
    ...
    this->in1     = new port(this, RECEIVE_PORT, 1, or_name + "_in1");
    this->in2     = new port(this, RECEIVE_PORT, 1, or_name + "_in2");
    this->out     = new port(this, SEND_PORT,    1, or_name + "_out");
}
...
MUX::MUX(int port_num, std::string name = "mux") {
    assert(port_num >= 1);
    ...
    for (int i=0; i<port_num; i++) {
        this->in.push_back(
                    new port(this, RECEIVE_PORT, SIGNAL_MAX, 
                                   mux_name + "_in" + std::to_string(i))
                );
    }
    this->ctrl = new port(this, RECEIVE_PORT, bits_required(port_num), 
                                mux_name + "_ctrl");
    this->out  = new port(this, SEND_PORT, SIGNAL_MAX, mux_name + "_out");
}
